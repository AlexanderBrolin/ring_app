...
**Обязательное чтение:** `docs/adr/ADR-001-Концепт.md` (геймдизайн, §4.1 — социальная
динамика: войс и короткие фразы обязательны в MVP), `docs/adr/ADR-002-Разработка.md`
(техрешения T1–T11, этапы, критические правила), `docs/adr/ADR-003-Сеттинг.md` (мир,
Директор, фракции, **словарь §9**), `docs/adr/SETUP-ПО.md` (тулинг). ADR — источник
истины; отклонение — только записанным amendment'ом. Правила клиентского трека —
`client/CLAUDE.md`; handoff клиентского трека — `docs/handoffs/client.md`.

**Номенклатура — по словарю ADR-003 §9** (сборщик, носитель, слепок, хозяин, Директор,
створ, перегруз, ведомые…): user-facing строки — русские термины словаря; идентификаторы
кода — их английские эквиваленты, зафиксированные на spec-фазе. Запрещённые синонимы
из чужих вселенных не использовать нигде, включая код, комментарии и bd.
...
1. Прочитать целиком: свой handoff (server-трек — `handoff_app_server.md` в локальном
   ...
   четыре файла `docs/adr/`, этот файл (+ `client/CLAUDE.md`, если работаешь в client/).
