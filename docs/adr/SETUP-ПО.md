...
| MetaVoiceChat | GitHub, **MIT, бесплатно**; вендоринг в `client/Assets/Plugins/MetaVoiceChat/` | Проксимити-войс (ADR-002 T11, амендмент гейта Ф10 — спека §6m Р204). Покупать НЕ нужно; Dissonance (~$75) — план Б, если интеграция окажется битой |
...
при конфликте действует amendment. ⚠ Секции у этого документа не было вовсе —
она заводится здесь по образцу ADR-001 §14, ADR-002 §10 и ADR-003 §11.

- **A1 (2026-09-03, эпик `app-88jb`, решение владельца Н10/Р361).** Уточняет
  строку **«Feel (More Mountains)»** в таблице «Пакеты в Unity-проект».
  **Hitstop из игры удалён целиком** (ADR-001 A14, ADR-002 A27), а он стоял
  первым из трёх поводов купить пакет. Оставшиеся два — тряска и вспышки —
  давно сделаны **своим кодом** и удаление hitstop пережили. ⇒ Оценка «окупает
  себя на Этапе 1» больше не действует, и пакет **не покупался**. Это замер, а
  не предположение: в `client/Assets/ThirdParty/` лежат четыре CC0-пака
  Quaternius и наши производные `_Ring`, ни `Feel`, ни `MoreMountains` там нет,
  и в `client/Packages/manifest.json` их тоже нет. Строка исходного текста
  остаётся записью о рассмотренном варианте; пометка «опционален» читается
  теперь как «отклонён».
