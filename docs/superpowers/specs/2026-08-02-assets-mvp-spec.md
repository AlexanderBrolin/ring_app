...
- 2026-08-02 (владелец, после инспекции паков): **1a** — ревизия T4/T6–T7 под
  фактическую поставку (один FBX на пак с отдельными takes; кукла внутри
  UAL1_Standard.fbx); **2a** — локомоция blend tree **1D по скорости**
  (стрейфов/8-dir в бесплатных Standard нет; §5-контракт скорректирован:
  Visual поворачивается к движению, прицел — Aim-слой), апгрейд на Pro возможен
  без переделки. Факты и отбор — assets-src/INSPECTION.md; ревизия Р-I в плане.
- 2026-08-02 (вехи-плейтесты 1–3, фидбек владельца): масштабы превью — мехи ×0.4,
  элита ×1.5 (Trilobite ×0.75), Директор ×3.5, роботы лицом к Сборщикам, EyeDrone
  парит (1.5 м); **Ual2Check = мужская кукла** (Mannequin_F невидим на сцене —
  дефект её Unity-экспорта; оставлена в репо со своим Avatar); DirectorSkin —
  свежий URP Lit с base+emissive картами пака (клон артефакта давал flat-red и
  предупреждение «материалы старой версии»); системный ремап текстур мехов/Locker
  (импорт не привязывал) — материалы в `_Ring/Materials/` + AddRemap; Speed=1 по
  умолчанию (Сборщик в превью бежит).
