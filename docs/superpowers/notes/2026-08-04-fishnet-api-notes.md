...
## Четыре наблюдения вертикального спайка (T3 Step 3, снято владельцем 2026-08-05)

Условия: два процесса Unity на разных копиях проекта, host + client, симулятор
задержки включён в обоих (`Latency = 40` — величина односторонняя, RTT = 80 мс
по CR 7; `Packet Loss = 0.05` в одну сторону ≈ 9.75% round-trip). Прогон ~1400
тиков (~47 с при 30 Гц). Приборы — оверлей спайка, разведённый по ролям после
фикс-волны Ф1.

**Показания процессов.** Host: `localTick 1393`, `RTT 167 ms`.
Client: `localTick 1581`, `RTT 133 ms`. Оба показывают
`sim: 40 ms one way (= 80 ms RTT) / 5% one way (= 9,75% round trip)`.

**Почему RTT не 80.** `TimeManager.RoundTripTime` документирован как «This value
includes latency from the tick rate» (`Runtime/Managing/Timing/TimeManager.cs:104`),
то есть к сетевому RTT прибавляется квантование тика: при 30 Гц до двух тиков
(≈ 67 мс). 80 + 33…67 = 113…147, и клиентские 133 мс ложатся в полосу. Симулятор
выставлен верно; сверять CR 7 нужно по строке `sim:`, а не по `RTT`.

### (а) Резинка

`correction: last 0,000 m, median 0,000 m (окно 128 сэмплов ≈ 4.3 с),
max 5,210 m (all time), samples 128`; отброшено из выборки: `repeated tick
stamp 0`, `no server data 0`; `server-sourced count 1470`.

**Вывод: в установившемся режиме предсказание совпадает с авторитетом бит в
бит.** Это ожидаемо по построению — клиентский шаг и серверный шаг исполняют
один и тот же код над одним и тем же инпутом (спайк, в отличие от продакшена,
не квантует инпут: Т25/Т34 введут кодек, и вот тогда паритет придётся
доказывать `PredictionParityTests`, Р34). `max 5,210 m` — единичный выброс за
всё время, по всем признакам стартовый: первая реконсиляция приходит на ещё не
инициализированную предсказанную копию. Ноль отброшенных подтверждает, что
фильтры фикс-волны (повтор штампа тика, реконсиляция без серверных данных)
не выбрасывают легитимные сэмплы.

### (б) `ReplicateState` при потере инпута

Снято с **серверной копии объекта удалённого клиента** (на копии владельца это
наблюдение недоступно по построению — FishNet всегда зовёт делегат один раз со
`Ticked|Created`):
`live+data 1276, live-no-data 8, replay+data 0, replay-no-data 0, future 0,
invalid 0, other 0 (mask Invalid), last Ticked, Created`.

**Вывод:** потерянный инпут выглядит как `Ticked` **без** `Created`
(корзина `live-no-data`) — за прогон таких 8. Корзина `other` пуста, то есть
неожиданных комбинаций флагов FishNet не выдал, и разбор по `Contains*`
покрывает весь наблюдаемый набор. Реплеев не было вовсе — согласуется с (а):
при нулевом расхождении переигрывать нечего.

**Оговорка к частоте:** 8 событий на ~1276 тиков — заметно чаще априорной
оценки `0.05³` (тройная избыточность инпутов, Р103). Точную модель потерь
спайк не проверял; для Т36 важно, что механизм голодания наблюдаем при штатных
5% и специально задирать потери не потребовалось.

### (в) Два `[Replicate]` за серверный тик

`replicates per server tick: max 1, >1 on 0 ticks, starved ticks 8`.

**Вывод: инвариант §3.7 подтверждён на живом трафике** — ровно один
`[Replicate]` потребляется за серверный тик, двойных не наблюдалось ни разу.
Восемь голоданий совпадают с восемью `live-no-data` из (б) — счётчики,
собранные независимо, сходятся. Ветка `InputOverwritten` при 80/5 не
активировалась; Т36 обязан её реализовать, но проверять придётся сценарием с
искусственным всплеском, а не штатным лагом.

### (г) Какой тик несёт `ReconcileData`

`reconcile tick 1566, localTick then 1580, delta 14, server-sourced count 1470`.

**Вывод:** реконсиляция несёт тик **потреблённого сервером инпута**
(`ClientStateTick`), а не текущее локальное время — как и предполагала спека
§3.9. Дельта — 14 тиков (≈ 466 мс при 30 Гц).

**ОТКРЫТЫЙ ВОПРОС для Т31/Т34:** 14 тиков заметно больше априорного ориентира
«RTT/2 + буфер интерполяции» (3–5 тиков). Одного снимка недостаточно, чтобы
отличить стабильное смещение (клиент штатно бежит впереди сервера, и тогда
ориентир был просто занижен) от дрейфа клиентских часов. Замерить динамику
дельты — обязательный пункт Т31, где часы рендера и строятся.

### Вердикт по стоп-условию фазы Ф1

Стоп-условие («модель `инпут → общий мир → реконсиляция из мира` несовместима с
конвейером FishNet») **не сработало**. Модель конвейером принята: серверная
ветка `[Replicate]` только записывает инпут, мир шагает ровно один раз на
`OnPostTick`, реконсиляция несёт полное состояние из авторитета, и предсказание
совпадает с ним бит в бит. Риск Р-А снят.

**Ценой одного найденного бага пакета** (Р110: опечатка `Unity.Mathmatics` в
