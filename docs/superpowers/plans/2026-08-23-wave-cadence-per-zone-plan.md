...
- **Мутация на каждую ветку:** мутации спеки §4 (M1–M13) распределены по
  ...
  и числом/механизмом ДО прогона**, пишет в `$SDD/task-ggvz-<N>-mutations-predicted.md`
  (имя с префиксом задачи: буквальное `task-<N>-…` столкнулось бы с семейством
  тасков Этапа 3 в том же каталоге).
...
- [ ] **Step 10:** R-COMPILE → чисто; R-TEST полный → красные по таблице;
      **время прогона записать** (§4 спеки: база — **153 c** чистого времени
      тестов, замерено сессией 46; находка — рост более чем вдвое, свыше 306 c).
