...
исполненным кодом)**. **План против §6l спеки — верить §6l.** **Поправка учёта Ф5:** штамп версии
застрял на `v3.1`, хотя правки фаз Ф2–Ф4 вносились; расхождение устранено.
Ниже — история до пофазной нумерации: v1 → self-review по `review_plan.md` четырьмя
...
**Гейт фазы Ф5:** R-TEST — **фактически +69 (327 → 396)**, не «≈ +22» (378 на
момент фазовых ревью, ещё 18 добавила фазовая фикс-волна): перевес
...
## Правки по гейту фазы Ф9 (v3.5 → v3.6, 2026-08-13)
...
1. **Т45 РАЗБИТ ВЛАДЕЛЬЦЕМ НА ТРИ ТАСКА** (Т45a — кукла и пул вьюх, Т45b —
   ...
   попадание). Объём вырос втрое против плана. Т47 разбит **трижды** тем же
   порядком: Т47a (признаки присутствия и живости в кадре), Т47b (наблюдение,
   свой труп в сети, HUD наблюдателя), Т47c (затухание на границе видимости).
   Списки Files и шаги плана для Т45 и Т47 читать как описание СУММЫ подтасков.

2. **Т46 исполнен ПРОТИВ БУКВЫ плана — решением владельца.** План (`:2056–2071`)
   и спека §3.3 требовали стены, непрострельной на любой высоте; вместо этого
   внутренние барьеры получили **моделируемую высоту** `Arena.BarrierTop` = 3.0 м
   (`app-r8x`), кандидат расщеплён на `HitBarrier`/`HitRingWall`, экран прицела
   переведён с камеры на **линию огня** (`Targeting.HasLineOfFire`). Спека §6l
   Р186.

3. **Аудит-блок Т47 в плане НЕВЕРЕН ЦЕЛИКОМ.** Он утверждает, что «в Presentation
   сегодня нет никакого понятия индекса игрока» и что греп даёт ноль совпадений —
   на момент исполнения вхождений больше десяти. Там же: буквальный переезд
   прицельного луча и `AimActive` на `ObservedIndex` **нарушил бы Р48** и вернул
   бы прицел в режим наблюдения; код исполняет Р48, луч и курсор остались на
   своём слоте. Спека §6l Р189.

4. **Т49 Step 1 требует не той сборки.** Написано «R-BUILD-`LinuxServer` +
   R-BUILD-`LinuxClient`»; для вехи В1 нужны **`LinuxClientDev` и
   `LinuxServerDev`** — на релизном плеере нет ни дев-оверлея, ни симулятора
   задержки, то есть CR 7 на нём неизмерим по построению, а симулятор применяет
   каждый процесс к своей исходящей, поэтому дев-сборки нужны с обеих сторон
   линка. Спека §6l Р192.

**Дополнение к гейту КАЖДОЙ фазы (Р191):** компиляционная часть гейта обязана
включать **хотя бы одну релизную цель сборки**. `app-9m4` показал цену обратного:
релизные сборки не компилировались всю фазу Ф9 начиная с первого таска, а
EditMode-прогон и R-COMPILE этого не видят в принципе — оба идут в редакторе, где
дев-символы определены. Гейт Ф9 исполнил правило собой: `BuildLinuxServer` и
