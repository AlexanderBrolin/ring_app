...
`docs/superpowers/specs/2026-08-04-stage2-network-spec.md` (**v3**).
...
- [ ] **Step 1:** проверить тег **не по памяти**:
...
- [ ] **Step 3:** R-COMPILE → EXIT=0, ГЕЙТ-ЛОГ пуст;
  `ls client/Library/PackageCache | grep -i fishnet`;
  `grep -i fishnet client/Packages/packages-lock.json` → пин зафиксирован.
- [ ] **Step 4:** R-TEST → **189/189**.
- [ ] **Step 5:** R-COMMIT `chore(app-5nu): Т1 — FishNet 4.7.2 через UPM-пин`.
...
- [ ] **Step 2:** зафиксировать **расхождения с текстом спеки** — каждое либо
  правка спеки отдельным `docs`-коммитом, либо находка владельцу.
- [ ] **Step 3:** R-COMMIT `docs(app-5nu): Т2 — сверка API FishNet по PackageCache`.
...
- [ ] **Step 3 (ручная проверка):** host + клиент в редакторе, симулятор 80/5
  включается **руками в инспекторе** (постоянный код — Т33). Записать в bd note:
  (а) резинка; (б) `ReplicateState` при потере инпута; (в) поведение при двух
  `[Replicate]` на серверный тик; (г) какой тик несёт `ReconcileData`.
- [ ] **Step 4:** ответы (а)–(г) — в заметку Т2.
- [ ] **Step 5:** R-COMPILE + R-TEST → 189/189.
- [ ] **Step 6:** R-COMMIT `feat(app-5nu): Т3 — вертикальный спайк предсказания`.
...
зелёные; визуальная проверка не проводится (стены появятся в грейбоксе в Т43);
...
- [ ] **Step 4:** R-FILTER `PvpDamageTests`+`ProjectileTests`+`HitZoneTests`+
  `DeathTests` → PASS; R-TEST → **golden не меняется** (в соло-сценарии второго
  игрока нет — ветка мёртвая; поехал → стоп).
...
**Гейт фазы Ф4:** R-TEST (≈ +8); golden не тронут; push; jsonl-chore; bd note;
...
**Гейт фазы Ф5:** R-TEST (≈ +22); golden не тронут; свип кириллицы; push;
jsonl-chore; bd note; `bd close` сабтаска Ф5.
...
Delete `.../Networking/Spike/**` **и `client/Assets/Scenes/NetSpike.unity`**
(вместе с `.meta`); Create `PredictionParityTests.cs` (+ `.meta`).
...
- [ ] **Step 4 (GREEN):** `Advance` с приёмником, `Step`, удаление `Spike/` и
  `NetSpike.unity`.
...
- [ ] **Step 2 (проверка):** дев-сборка клиента — в оверлее видно «80 мс / 5%»;
...
| Ф4 PvP | Т17–Т18 | ноль аллокаций на троих | не тронут |
...
**Инвариант:** golden двигается **только** в Т10 и Т16. Любой другой сдвиг —
стоп и разбор.
