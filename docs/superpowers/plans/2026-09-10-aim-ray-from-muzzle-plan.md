...
- Modify: `client/Assets/Tests/EditMode/TestConfigs.cs` — ⚠ **обязателен, и это запись 14
  «Отклонений»** (спека §3.1 его в изменяемых не числит): подъём `PutObstacle`/`PutWall` для
  тестов 8 и 13, иначе барьер ставить нечем — `Open()` обнуляет `ObstacleCount`/`WallCount`
  (`:676-682`)
- Modify: `client/Assets/Tests/EditMode/BarrierHeightTests.cs` — ⚠ **следствие подъёма**:
  приватная пара (`:72`, `:79`) снимается, **шестнадцать** её вызовов в этом же файле
  переводятся на `TestConfigs`; иначе подъём заводит третью копию, которую сам же запрещает
...
- [ ] **Step 1 (RED, первая группа — геометрия и дальность: тесты 1, 2, 3, 4, 16, 17, 18):**
      ...
      ⚠ **Разбиение на три группы — правка круга 1 по гранулярности:** 22 фикстуры с ручной
      геометрией одним шагом — это часы, а не пять минут, и у каждой группы должен быть свой
      verify-RED.
      ⛔⛔ **ПОРЯДОК ШАГОВ ИМЕННО ТАКОЙ, И ЭТО ПРАВКА КРУГА 2:** заглушки (Step 2) кладутся
      **вместе с ПЕРВОЙ группой**, иначе её собственный verify-RED невозможен — файл не
      скомпилируется. ⇒ фактическая последовательность: **Step 1 (фикстуры 1-й группы) + Step 2
      (заглушки) → Step 3 verify (7/7) → Step 1a → verify (19/19) → Step 1b → verify (22/22)**.
      Нумерация ниже сохранена для ссылок, но verify после каждой группы — обязателен.
...
⚠ **`MaxHalfAngleRad` обязан быть виден тесту** — он `public const` в `Spread` до T5 (и
`internal` вместе с классом после; `InternalsVisibleTo("Ring.Simulation.Tests")` уже стоит). ⛔ **Литералом его в тест не писать**
...
/// ⚠ Суффикс Rad — конвенция сборки для ПОЛЕЙ и констант (SpreadRad,
/// RecoilMaxRad, RecoilPerShotRad, DoorCenterRad); у методов конвенция своя —
/// Radians (HipRadians, AimRadians). Величина в радианах без суффикса
/// читается как градусы.
/// ⚠ PUBLIC, А НЕ INTERNAL, ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И HipHalfWidth (запись 9
/// «Отклонений», находка круга 2): до T5 класс ещё public, и internal-член
/// в нём был бы тем самым расколом одного конуса по двум уровням доступа.
/// Все константы публичных классов сборки публичны — Geometry.Skin (:8),
/// Impact.RestEpsilon (:37). Понижается вместе с классом в T5.
public const float MaxHalfAngleRad = 1.5533430f;   // 89 градусов
...
- [ ] **Step 5:** R-COMPILE → `EXIT=0`; R-APPLY → `EXIT=0` + ГЕЙТ-ЛОГ; **YAML-проверка `Main.unity`**:
      ...
      что у луча.
- [ ] **Step 6:** R-COMMIT `chore(app-461s): T3 — два чайлда засечек в Main.unity (бутстрап)`
      → **R-IDEM** → `git status --porcelain -- client/` и `git diff -- client/` пусты.
      ⚠ **Коммит артефактов идёт полным R-COMMIT** (свип, секрет-чек, ГЕЙТ-META, сверка скоупа),
      а не «просто коммитом»: у него та же конвенция типа и текста, что у остальных. ⛔ **Тестов бутстрапа в наборе нет вовсе** — это единственный
...
// ⛔ ДОЛЯ, А НЕ АБСОЛЮТ: полный зазор между засечками ДО ОТКРЫТИЯ ОГНЯ на
// десяти метрах равен 0.52 м (полуширина 0.26 в каждую сторону), и абсолютный
// штрих в 0.35 м оставил бы между внутренними концами всего 0.17 м — пара
// читалась бы одной меткой ровно тогда, когда решение и принимается, до
// первого выстрела. ⚠ Полное слияние наступает при stroke >= 2 x hw = 0.52 м
// (уточнение круга 2: v2 говорила «слил бы», не назвав порога).
...
- [ ] **Step 6:** R-COMMIT `chore(app-461s): T4 — три ручки целеуказателя доставлены в
      GameFeelConfig.asset` → **R-IDEM** → пусто.
...
- [ ] **Step 2 (`CrosshairView`):** снять пять сущностей конуса **и** `using`; весь блок маркера
      ...
      ⚠ **И класс-док бутстрапа станет ложным третьим местом** (находка круга 2):
      `StageOneSceneBootstrap.cs:97` описывает объект конуса как «`CrosshairView.ConeSegments`
      points». Свип T6 ищет только `WeaponSystem` и эту строку не найдёт — правится здесь.
...
- [ ] **Step 5:** R-COMPILE → `EXIT=0` + ГЕЙТ-ЛОГ; R-APPLY → `EXIT=0`;
      **YAML-проверка `Main.unity`**: объекта `SpreadCone` нет, набор `m_Name` в остальном не
      двинулся (правило 14).
- [ ] **Step 6:** R-COMMIT `chore(app-461s): T5 — объект SpreadCone удалён из Main.unity`
      → **R-IDEM** → пусто. ⛔ **И R-IDEM здесь важнее обычного:**
...
- [ ] **Step 1 (замеры ДО правки — они идут В ТЕКСТ амендмента):**
      ...
      **метафора**, а не термин, и §6.2 handoff'а числит её законным совпадением);
      ...
      🆕 ⛔⛔ **И СВИП ИМЁН, ЗАВЕДЁННЫХ КРУГАМИ РЕВЬЮ, — ОТДЕЛЬНОЙ СТРОКОЙ** (находка круга 2:
      их в круге 1 не было, значит по словарю их не свипали вовсе, а урок 724 ровно об этом):
      `/usr/bin/grep -rn "Stroke\|Drawn\|Scratch\|MuzzleSimPos\|HalfAngle\|AimStop\|EnsureChild" docs/adr/`
      → **ожидание ноль** (проверено кругом: ноль и по ADR, и по коду для `MaxHalfAngle`,
      `NotchStroke`, `EnsureChild`, `SetDrawn`; `Scratch` и `Drawn` в коде заняты законно —
      `_expiredScratch`, `_lastInputsScratch`, `_boardDrawn` — и конвенции не спорят).
      Число записывается в амендмент, как это делают A7/A8.
...
8. ⛔⛔ **ДЛИНА ШТРИХА ЗАСЕЧКИ — ЧИСТАЯ ФУНКЦИЯ `AimLine.NotchStroke`, А НЕ ВЫРАЖЕНИЕ В ВИДЕ.**
   ...
   клала в `AimLineTests` — **в место, которое этой формулы не видит**: `Notches` принимает
   `strokeLength` уже готовым, а сама формула живёт в `MonoBehaviour`, до которого EditMode не
   достаёт. ⚠ **Довод про видимость `GameFeelConfig` тестам был неверен и круг 2 его снял:**
   asmdef `Ring.Simulation.Tests` ссылается на `Ring.Data`, то есть тестам он **виден**; невидим
   он **боевой** сборке `Ring.Simulation` (её asmdef ссылается только на `Unity.Mathematics`,
   CR 1) — вот из-за чего формулу нельзя было бы позвать из `AimLine`, если бы она осталась
   числом из SO.
...
14. **ПРАВЯТСЯ `TestConfigs.cs` И `BarrierHeightTests.cs`, КОТОРЫХ СПЕКА §3.1 В ИЗМЕНЯЕМЫХ НЕ
   ЧИСЛИТ.** Тесты 8 и 13 требуют барьера, а `TestConfigs.Open()` обнуляет `ObstacleCount` и
   `WallCount` (`:676-682`) — ставить `BarrierTop` мало, барьер надо **создать**. Готовая пара
   `PutObstacle`/`PutWall` живёт приватными статиками `BarrierHeightTests` (`:72`, `:79`) ⇒
   поднимается в `TestConfigs`, и **шестнадцать** её вызовов в исходном файле переводятся туда же.
   ⚠ **И сказано, чего подъём НЕ делает:** постановка препятствия руками живёт в наборе **54 раза
   в 16 файлах**, так что правило 2 он закрывает частично — не заводит семнадцатый дом и даёт
   общий; свип остальных пятнадцати — отдельная задача, а не этот заход.
