...
> **Post-plan amendments (вехи, 2026-08-04):** владелец принял четыре
> решения ПОСЛЕ этого плана (полностью — spec §6a): (1) экономика Буста
> переработана — пул 100, дэш 40, слайд 30, `LinkedDashStaminaCost` удалён,
> `LinkRefund 10` в оба окна связки, КД дэша остаётся и обходится только
> дэшем-в-окне, реген 20/0.8; (2) слайд-анимация — UAL2-сет вместо Mixamo
> (ADR-002 A10 amendment); (3) обломки — настоящие Blender-split части
> (`app-1zf`: меши монолитны; примитив-фолбэк заменён), хедшот = всегда
> развал с отлётом головы; (4) B5 headless-труп снят (монолитные меши,
> арт-issue `app-vli`). Тело плана ниже — v4/v5, НЕ переписано под них.
