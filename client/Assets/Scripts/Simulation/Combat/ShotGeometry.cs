...
namespace Ring.Simulation.Combat
{
    ...
    ///
    /// ⚠ PUBLIC, UNLIKE ITS NEIGHBOR RewindSplit, AND THAT IS DELIBERATE
    /// RATHER THAN AN OVERSIGHT: the actual consumer is `Ring.Presentation.
    /// Net`'s `NetworkSimBackend`, which reads a predicted shot's own
    /// `PredictedShot.Solution` off `PredictedShotLog` to feed
    /// `TracerProjectiles`, and that assembly references Ring.Simulation.
    /// RewindSplit is internal because nobody outside the simulation waits
    /// for it. The "PUBLIC FOR EXACTLY TWO MEMBERS" discipline in
    /// WeaponSystem's own header is not broken by this -- that rule is about
    /// WeaponSystem, and it is exactly why the geometry moves into a file of
    /// its own instead of becoming a third public member there.
    ///
