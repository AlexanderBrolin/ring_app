...
namespace Ring.Simulation.Tests
{
    ...
    /// Id 1..5, SlotCost 1/2/3/4/1 — coordinator fix-round Ф3 review C1
    /// shifted every id up by one, 0 is now reserved as the container
    /// slot's own "empty" sentinel) — Loot.Inventory.SlotCostOf now
    /// resolves through the real catalog (ItemCatalogLookup.Find, R-89), so
    /// an id outside it throws rather than defaulting to 1 the way the
    /// removed T4 -> T13 stub silently did.
    public class InventoryTests
    {
        ...
        {
            // Id 1 costs 1, Id 2 costs 2 (spec §3.7 table: Т1/Т2) — chosen
            ...
            var w = new SimulationWorld(1, TestConfigs.Default());
            Assert.IsTrue(w.TryAddItem(0, 1), "premise: the first add (Id 1) must succeed");
            Assert.IsTrue(w.TryAddItem(0, 2), "premise: the second add (Id 2) must succeed");

            Assert.AreEqual(3, w.InventoryUsedSlots(0),
                "Id 1 (cost 1) plus Id 2 (cost 2) must total 3 slot points");
        ...
        {
            // Id 1 and Id 5 (the repair kit) both cost 1 (spec §3.7 table),
            // so two of them exactly fill a capacity of 2.
            var cfg = TestConfigs.Default();
            cfg.Hero.InventoryCapacity = 2;
            var w = new SimulationWorld(1, cfg);
            Assert.IsTrue(w.TryAddItem(0, 1), "premise: the first item (Id 1, cost 1) must fit");
            Assert.IsTrue(w.TryAddItem(0, 5), "premise: the second item (Id 5, cost 1) must exactly fill capacity");

            bool addedThird = w.TryAddItem(0, 1);
        ...
        public void AddBeyondMaxItems_Refused_AndInventoryUnchanged()
        {
            ...
            // reach. Same item id (1) added three times on purpose: this
            ...
            Assert.IsTrue(w.TryAddItem(0, 1), "premise: the second item must exactly fill the array ceiling");
        ...
        public void InventoriesOfPlayers_DoNotMix()
        {
            ...
            Assert.IsTrue(w.TryAddItem(1, 1), "premise: the add to player 1's backpack must succeed");
            ...
            Assert.AreEqual((byte)1, w.InventoryItemAt(1, 0), "…and be the exact item that was added");
        }
        ...
        public void Clear_EmptiesTheBackpack()
        {
            ...
            Assert.IsTrue(inv.TryAdd(1, capacity: 8, catalog), "premise: the first item must actually fit");
            Assert.IsTrue(inv.TryAdd(2, capacity: 8, catalog), "premise: the second item must actually fit");
